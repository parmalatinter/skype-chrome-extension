...
(function ($) {
    ...
    $.fn.timepicki = function (options) {
        ...
        var defaults = {
            format_output: function (target, tim, mini, sec, meri) {
                if (settings.show_meridian) {
                    ...
                    $(target + '.meri').val(meri);
                    $(target + '.trim').val(tim.toString());
                    $(target + '.min').val(mini.toString());
                    $(target + '.sec').val(sec.toString());
                    return tim + ":" + mini + ":" + meri + ":" + sec;
                } else {
        ...
        return this.each(function () {
            ...
            var new_ele = $(
                ...
                "<div class='secs'>" +
                top_arrow_button +
                "<div class='sec_tx'><input type='text' class='timepicki-input'" + (settings.disable_keyboard_mobile ? "readonly" : "") + "></div>" +
                bottom_arrow_button +
                "</div>");
            if (settings.show_meridian) {
                new_ele.append(
            ...
            $(".timepicki-input").keydown(function (keyevent) {
                ...
                function validate() {
                    ...
                    if (grandfatherDiv.hasClass("time")) { /// HOUR
                    ...
                    } else if (grandfatherDiv.hasClass("secs")) { /// MINUTE
                        ...
                        if (isValidNumber) {
                            ...
                            // clip number.
                            var secs = Math.min(Math.max(parseInt(input.val()), 0), 59);
            ...
            inputs.on('keydown', function (e) {
                ...
                if (input.closest('.timepicker_wrap .time').length) {
                    ...
                    change_times(null, 'mins', direction);
                } else if (input.closest('.timepicker_wrap .secs').length) {
                    change_times(null, 'secs', direction);
            ...
            function set_value(event, close) {
                ...
                var sec = ele_next.find(".sec_tx input").val();
                var meri = "";
                if (settings.show_meridian) {
                    meri = ele_next.find(".mer_tx input").val();
                }

                if (tim.length !== 0 && mini.length !== 0 && sec.length !== 0 && (!settings.show_meridian || meri.length !== 0)) {
                    ...
                    if (settings.show_meridian) {
                        ...
                        ele.val(settings.format_output(ele.attr('target'), tim, mini, sec, meri));
                    } else {
                        ele.val(settings.format_output(ele.attr('target'), tim, mini, sec));
                    }
                }
            ...
            function set_date(start_time) {
                ...
                if (ele.is('[data-timepicki-tim]')) {
                    ...
                    sec = Number(ele.attr('data-timepicki-sec'));
                    if (settings.show_meridian) {
                ...
                if (sec < 10) {
                    ele_next.find(".sec_tx input").val("0" + sec);
                } else {
                    ele_next.find(".sec_tx input").val(sec);
                }
                if (settings.show_meridian) {
                    if (mer < 10) {
            ...
            function change_times(cur_ele, type, direction) {
                var cur_clis = ["mins", "secs"];
                var cur_classies = ["mi", "sec"];
                var ele_st = 0;
                var ele_en = 59;
                $.each(cur_clis, function (key, cur_cli) {
                    if (type === cur_cli) {
                        var cur_class = cur_classies[key];
                        var cur = Number(ele_next.find("." + cur_cli + " ." + cur_class + "_tx input").val());
                        var step_size = Number(settings.step_size_minutes);
                        if ((cur_ele && cur_ele.hasClass('action-next')) || direction === 'next') {
                            if (cur + step_size > ele_en) {
                                ele_next.find("." + cur_cli + " ." + cur_class + "_tx input").val("00");
                                ...
                                cur = cur + step_size;
                                if (cur < 10) {
                                    ele_next.find("." + cur_cli + " ." + cur_class + "_tx input").val("0" + cur);
                                } else {
                                    ele_next.find("." + cur_cli + " ." + cur_class + "_tx input").val(cur);
                                }
                            }
                        } else if ((cur_ele && cur_ele.hasClass('action-prev')) || direction === 'prev') {
                            if (cur - step_size <= -1) {
                                ele_next.find("." + cur_cli + " ." + cur_class + "_tx input").val(ele_en + 1 - step_size);
                                if (settings.overflow_minutes) {
            ...
            $(cur_prev).add(cur_next).on("click", function () {
                ...
                if (cur_ele.parent().attr("class") == "time") {
                    ...
                    change_times(cur_ele, "mins");
                } else if (cur_ele.parent().attr("class") == "secs") {
                    change_times(cur_ele, "secs");
                } else {
                    if (settings.show_meridian) {
                        change_meri(cur_ele);
                    }
